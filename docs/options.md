...
| Hide tool calls | bool | `false` | Hide the tool rows — commands, file reads and edits, searches, MCP calls, sub-agents — so the transcript reads as your messages and Claude's replies. Your answers to questions, plan decisions, the task list and a call waiting for your approval stay, and so do thinking blocks. The filter button next to Session History on the chat toolbar flips the same setting for every open chat, and brings the rows back as they were — open or closed, older history kept. Changed here instead, it reloads each idle chat like any other setting on this page. |
