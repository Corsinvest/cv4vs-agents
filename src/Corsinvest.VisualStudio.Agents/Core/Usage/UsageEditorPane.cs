...
/// <summary>Document pane for the Usage tab. Frameless + read-only: no backing file, so every
/// IVsPersistDocData / IPersistFileFormat member is a no-op (never dirty, never saves, no reload).
/// The content is UsageControl, which fetches usage live per profile. The factory returns this same
/// object as both doc view and doc data.</summary>
internal sealed class UsageEditorPane : WindowPane, IVsPersistDocData, IPersistFileFormat
{
    // The placeholder file VS opened. Held only so GetCurFile can return it (the RDT moniker); its
    // content is never read — the UI fetches usage from the CLI.
    private string _fileName = "";

    public UsageEditorPane() : base(null)
    {
    ...
    {
        pClassID = PackageGuids.UsageEditorFactory;
    ...
    int IPersistFileFormat.Load(string pszFilename, uint grfMode, int fReadOnly)
    {
        ...
        // Content is never read from disk — the UI fetches usage from the CLI.
        return VSConstants.S_OK;
    }
